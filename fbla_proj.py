...
main.iconphoto(False, aai_image1)
...
def sign_up():
  ...
  sign_up_window.iconphoto(False, aai_image1)
...
lib1.place(relx=0.5, rely=0.41, anchor=tk.CENTER)
# tool-tip adds add-on help to buttons when hovered over #
lib1_tip = ToolTip(lib1, msg="Logs User In", delay = 1.5)

# sign-up button #
sub1 = tk.Button(main, text="Sign Up", command=sign_up)
sub1.place(relx=0.5, rely=0.47, anchor=tk.CENTER)
# tool-tip adds add-on help to buttons when hovered over #
sub1_tip = ToolTip(sub1, msg="opens signup \n window", delay = 1.5)

aai_image_label = tk.Label(main, image=aai_image2, bg="cornflowerblue")
aai_image_label.place(relx=0.5, rely=0.7, anchor=tk.CENTER)
...
def data_window():
  ...
  def info_box_window():
    ...
    info_window.iconphoto(False, aai_image1)
    ...
    image_label = tk.Label(info_window, image=(partner_image_dict[final_partner]), bg="powderblue")
  ...
  def add_window():
    ...
    add_window.iconphoto(False, aai_image1)
  ...
  def delete_window():
    ...
    delete_window.iconphoto(False, aai_image1)
    ...
    for i in temp_filter_result:
      for j in i:
        if j != []:
          ...
          h = list(j)
          filter_listbox_result = (h[0] + " " + ("-"*(30-(len(h[0])))) + " " + h[5])
  ...
  try:
    ...
    if (list(result))[1] == pass_var_login.get(): 
      ...
      data_window.iconphoto(False, aai_image1)
      # Menu Bar #
      ...
      if str(user_perm.get()) == "admin":
        ...
        menu_3.add_command(label="Remove Item", command=lambda:[delete_window()], image=minus_image, compound="left")
        
        # lets you view all users and pending users #
        menu_3.add_command(label="All Users", command=view_users, image=user_image, compound="left")
      ...
      searchbutton = tk.Button(data_window, command=lambda: [search_for_organization()], text='Search', image=search_image, compound="left")
      ...
      info_button.place(width=70, height=25, relx=0.78, rely=0.094)
